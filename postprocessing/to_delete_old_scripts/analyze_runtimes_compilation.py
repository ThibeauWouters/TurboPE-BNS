...
def body(args) -> float:
    ...
    This version returns the runtime of the initialization and compilation, in order to find the total time of training the NF and sampling only.
    """
    ...
    while network_snr < args.SNR_threshold:
        ...
        ifos = [H1, L1, V1]
        psd_dir = "../data/psds/"
        psd_files = [f"{psd_dir}psd.txt", f"{psd_dir}psd.txt", f"{psd_dir}psd_virgo.txt"]
